...
import { ROLE_ADMIN, ROLE_INVITE, ROLE_MEMBRE } from "../../services/apiConfig";
...
export default function WidgetEquipe({ onClose }: { onClose?: () => void; isGuest?: boolean }) {
  ...
  const MemberCard = ({ member, icon, colorClass, bgColorClass, borderColorClass }: { 
    member: MembreDTO, 
    ...
    <div className={`flex items-center gap-3 p-2.5 bg-white rounded-xl border ${borderColorClass} shadow-sm hover:shadow-md hover:translate-x-1 transition-all group`}>
      <div className={`flex items-center justify-center w-10 h-10 rounded-full ${bgColorClass} ${colorClass} font-bold text-sm shrink-0 border border-white shadow-inner`}>
        {member.prenom?.charAt(0).toUpperCase() || member.nom?.charAt(0).toUpperCase() || "?"}
      </div>
      <div className="flex flex-col min-w-0">
        <span className="text-sm font-semibold text-slate-800 truncate">
          {member.prenom} <span className="uppercase text-[10px] opacity-70">{member.nom}</span>
        </span>
        <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
          {icon}
          <span className="text-[10px] font-medium tracking-wider uppercase">
            {colorClass.includes('orange') ? 'Admin' : colorClass.includes('blue') ? 'Membre' : 'Invité'}
          </span>
        </div>
      </div>
    </div>
